...
class MediaBackup
{
    ...
    static string backupFolder = "backups"; // название папки бекапа
    ...
    static void Main()
    {
        ...
        while (true)
        {
            ...
            {
                isInRecoveryMode = CheckIfInRecoveryMode();
                ShowMainMenu();
        ...
        static bool CheckIfInRecoveryMode()
        {
            ...
            return output.Contains("recovery");
        }
        ...
        static void ShowMainMenu()
        {
            while (true)
            {
                ...
                Console.WriteLine(new string('-', 72));
                Console.WriteLine("https://github.com/keyx7/ABMT");
        ...
        static void DisplayConnectionStatus()
        {
            ...
            if (!string.IsNullOrEmpty(output))
            {
                ...
                foreach (var line in deviceLines)
                {
                    ...
                    {
                        status = rm.GetString("ConnectionStatus.Recovery");
                        break;
                    }
                }
            }
        ...
        static void BackupMedia()
        {
            ...
            List<(long size, string path)> files;

            if (isInRecoveryMode)
            {
                dataPath = "/sdcard/"; // Изменение android/data путя в зависимости от режима подключения
                files = GetFilesInRecoveryMode();
            }
            else
            {
                dataPath = "/storage/emulated/0/";
                files = GetFilesInNormalMode();
            }

            // Подсчёт файлов в Android/data/ и других путях
            var dataFiles = files.Where(file => file.path.StartsWith(dataPath + "Android/data/")).ToList();
            var otherFiles = files.Where(file => !file.path.StartsWith(dataPath + "Android/data/")).ToList();
            ...
            long totalSizeForArchiving = new DirectoryInfo(tempBackupFolder).GetFiles("*", SearchOption.AllDirectories).Sum(f => f.Length);
            long archivedSize = 0;
        ...
        static List<(long size, string path)> GetFilesInRecoveryMode()
        {
            var files = new List<(long size, string path)>();
            {
                var findProcess = new Process
                {
                    ...
                    {
                        FileName = adbPath,
                        Arguments = $"shell find /sdcard -type f \\( " +
                ...
                files.AddRange(output.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                    .Select(line =>
                    {
        ...
        static List<(long size, string path)> GetFilesInNormalMode()
        {
            var findProcess = new Process
            {
                StartInfo = new ProcessStartInfo
            ...
            {
                Console.WriteLine($"ADB Error: {errorOutput}");
                return new List<(long size, string path)>();
            }
